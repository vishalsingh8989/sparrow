...
public class Scheduler {
  ...
  public void handleJobSubmission(TSchedulingRequest request) throws TException {
    ...
    Iterator<InetSocketAddress> iterator = state.getBackends(app).iterator();

    while(iterator.hasNext()){
        InetSocketAddress mInetSocketAddress = iterator.next();
        THostPort host = new THostPort(mInetSocketAddress.getAddress().getHostAddress(), mInetSocketAddress.getPort());
        mDynamicScheduler.addBackend(host);
    }

    LOG.debug("NumBackends: " + backends.size());
    LOG.debug("NumBackends info: " + backends.toArray());
...
// Do not return error client to pool
    ...
    public void sendFrontendMessage(String app, TFullTaskId taskId,
        ...
        LOG.debug(Logging.functionCall(app, taskId,status, message));
        LOG.info("frontendSockets : "  + frontendSockets.values().toString());
        LOG.info("message : "  + message.getChar(0));


        LOG.info("getTaskStatus : " + mDynamicScheduler.getTaskStatus(taskId.requestId) + ", " + taskId.schedulerAddress);
